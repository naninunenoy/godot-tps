...
if (-not $f -or -not ($f.EndsWith('.cs') -or $f.EndsWith('.csproj'))) {
    exit 0
}

$errs = @()

dotnet format tps.slnx 2>&1 | Out-Null
dotnet csharpier format . 2>&1 | Out-Null
...
if ($errs.Count -gt 0) {
    @{
        hookSpecificOutput = @{
            ...
            additionalContext = ($errs -join [char]10)
        }
