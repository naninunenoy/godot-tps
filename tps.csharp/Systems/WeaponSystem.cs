...
public sealed class WeaponSystem(World world, Router router, ILogStore? logStore = null)
{
    public bool TryFire(EntityId id)
    {
        ...
        logStore?.Add(new GameLogEntry(
            ...
            new Dictionary<string, object?> { ["EntityId"] = id.AsPrimitive(), ["AmmoLeft"] = ammoLeft },
    ...
    public void Update(EntityId id, float delta)
    {
        ...
        world.SetComponent(id, weapon with { ReloadTimer = reloadTimer, FireCooldown = fireCooldown, Ammo = ammo });

        if (reloadCompleted)
            logStore?.Add(new GameLogEntry(
                LogLevel.Information,
                GameEvents.ReloadCompleted,
                new Dictionary<string, object?> { ["EntityId"] = id.AsPrimitive(), ["Ammo"] = ammo },
                DateTimeOffset.UtcNow));
    }
}
